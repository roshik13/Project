...
def hog1(feat1,mag3,ang3):
  fd, hog_image = hog(feat1, orientations=9, pixels_per_cell=(8, 8),
          cells_per_block=(2, 2), visualize=True, multichannel=True)
  cv2.imwrite('behavehog.png',hog_image)
  return bow(fd)
...
vid=['35450-47160.avi','47300-58400 (1).avi']
X1=[]
Y=[]
acc2=0
nocount=0
...
for video in range(0,2):
  ...
  while success:
      ...
      if success and couy%2==0 and couy>35650:
          county=county+1
          print(couy)
          results=model(frame)
          ...
          if X!=[]:   
            ...
            Y.append(couy)
            if video==0:
              if couy in range(41899,42000) or couy in range(44447,44577) or couy in range(46375,46614):
                y.append(1)
              else:
                y.append(0)
            elif video==1:
              if couy in range(50727,51051) or couy in range(52368,52759) or couy in range(54714,55005):
                y.append(1)
              else:
                y.append(0)
            X=[]
...
Xmain=[]
Ymain=[]
for i in range(0,len(X1)-15):
     sum=0
     NH=[]
     for j in range(0,16):
          NH.append(X1[j+i])
          sum=sum+y[j+i]
     Xmain.append(list(NH))
     if sum>4:
          Ymain.append(1)
     else:
          Ymain.append(0)
Xmain=np.asarray(Xmain).astype('float32')
Ymain=np.asarray(Ymain).reshape(-1,1)
Xmain=Xmain.reshape(-1,16,100,1)
batch_size = 20
Ymain=to_categorical(Ymain)
train_X,valid_X,train_label,valid_label = train_test_split(Xmain, Ymain, test_size=0.3, random_state=13)
#Ymain=to_categorical(Ymain)
...
fashion_model.compile(loss=keras.losses.binary_crossentropy, optimizer=keras.optimizers.Adam(learning_rate=0.001),metrics=['accuracy'])
